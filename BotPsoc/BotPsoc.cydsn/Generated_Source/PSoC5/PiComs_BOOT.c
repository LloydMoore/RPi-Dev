...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_PiComs) || \
                                          (CYDEV_BOOTLOADER_IO_COMP == CyBtldr_Custom_Interface)))


/*******************************************************************************
* Function Name: PiComs_CyBtldrCommStart
********************************************************************************
*
* Summary:
...
void PiComs_CyBtldrCommStart(void) CYSMALL 
{
    ...
    PiComs_Start();
    PiComs_ClearRxBuffer();
    PiComs_ClearTxBuffer();
}


/*******************************************************************************
* Function Name: PiComs_CyBtldrCommStop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: PiComs_CyBtldrCommReset
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: PiComs_CyBtldrCommWrite
********************************************************************************
*
* Summary:
         ...
         
{
    ...
    (void) PiComs_ReadTxStatus();

    /* Write TX data using blocking function */
    #if(0u != PiComs_MODE_USE_ZERO)
        PiComs_WriteTxDataZero(pData[buf_index]);
        buf_index++;
    #endif /* (PiComs_MODE_USE_ZERO == 1u) */

    /* Write TX data using blocking function */
    do
    {
        PiComs_WriteTxData(pData[buf_index]);
        buf_index++;
    }while(buf_index < size);

    while(0u != timeoutMs)
    {
        if(0u != (PiComs_ReadTxStatus() & PiComs_STS_SPI_DONE))
        {
            /* Return success code */
            *count = size;
...
/*******************************************************************************
* Function Name: PiComs_CyBtldrCommRead
********************************************************************************
*
* Summary:
         ...
         
{
        ...
        */
        if(0u != PiComs_GetRxBufferSize())
        {
            ...
            */
            do
            {
                oldDataCount = PiComs_GetRxBufferSize();
                CyDelay(PiComs_BYTE2BYTE_TIME_OUT);
            }while(PiComs_GetRxBufferSize() > oldDataCount);
            status = CYRET_SUCCESS;
            break;
        }
    ...
    *count = 0u;
    ...
    while(PiComs_GetRxBufferSize() > 0u)
    {
        tempCount = PiComs_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = PiComs_ReadRxData();
                dataIndexCntr++;
            }
...
#endif /* if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_PiComs) || \
                                          (CYDEV_BOOTLOADER_IO_COMP == CyBtldr_Custom_Interface)) */
