...
void PiSPI_CLK_Start(void) 
{
    ...
    PiSPI_CLK_CLKEN |= PiSPI_CLK_CLKEN_MASK;
	PiSPI_CLK_CLKSTBY |= PiSPI_CLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: PiSPI_CLK_Stop
********************************************************************************
*
* Summary:
...
void PiSPI_CLK_Stop(void) 
{
    ...
    PiSPI_CLK_CLKEN &= (uint8)(~PiSPI_CLK_CLKEN_MASK);
	PiSPI_CLK_CLKSTBY &= (uint8)(~PiSPI_CLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: PiSPI_CLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((PiSPI_CLK_CLKEN & PiSPI_CLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = PiSPI_CLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = PiSPI_CLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* PiSPI_CLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(PiSPI_CLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(PiSPI_CLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: PiSPI_CLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: PiSPI_CLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = PiSPI_CLK_GetSourceRegister();
    uint16 oldDivider = PiSPI_CLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = PiSPI_CLK_CLKEN & PiSPI_CLK_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((PiSPI_CLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: PiSPI_CLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(PiSPI_CLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: PiSPI_CLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    PiSPI_CLK_MOD_SRC |= modeBitMask & (uint8)PiSPI_CLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: PiSPI_CLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    PiSPI_CLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(PiSPI_CLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: PiSPI_CLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return PiSPI_CLK_MOD_SRC & (uint8)(PiSPI_CLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: PiSPI_CLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = PiSPI_CLK_GetDividerRegister();
    uint8 oldSrc = PiSPI_CLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        PiSPI_CLK_MOD_SRC |= CYCLK_SSS;
        PiSPI_CLK_MOD_SRC =
            (PiSPI_CLK_MOD_SRC & (uint8)(~PiSPI_CLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: PiSPI_CLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return PiSPI_CLK_MOD_SRC & PiSPI_CLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: PiSPI_CLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    PiSPI_CLK_PHASE = clkPhase & PiSPI_CLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: PiSPI_CLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return PiSPI_CLK_PHASE & PiSPI_CLK_PHASE_MASK;
}
